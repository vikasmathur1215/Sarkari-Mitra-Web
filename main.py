...
# 4. Chat History
# --- Quick Buttons (Updated with AI Trigger) ---
st.write("---") 
col1, col2, col3 = st.columns(3)

with col1:
    if st.button("📄 Resume"):
        prompt = "मुझे एक प्रोफेशनल Resume बनाना है, मेरी मदद करो।"
        st.session_state.messages.append({"role": "user", "content": prompt})
        # यह लाइन एआई को तुरंत जगा देगी
        st.rerun()

with col2:
    if st.button("🏠 Rent Draft"):
        prompt = "मुझे एक Rent Agreement का ड्राफ्ट बना कर दो।"
        st.session_state.messages.append({"role": "user", "content": prompt})
        st.rerun()

with col3:
    if st.button("📜 Affidavit"):
        prompt = "मुझे एक सामान्य Affidavit का फॉर्मेट चाहिए।"
        st.session_state.messages.append({"role": "user", "content": prompt})
        st.rerun()
